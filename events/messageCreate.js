        ...
        if (await db.get(`afk_${message.author.id}`)) {
            ...
            if (afkDate && sebep) {
                const date = new EmbedBuilder()
                    .setDescription(`:wave: ${message.author} Hoş geldin! **${sebep}** sebebiyle <t:${parseInt(afkDate.date / 1000)}:R> afk'ydın.`)
        ...
        if (kullanıcı) {
            ...
            if (sebep) {
                const sebeps = new EmbedBuilder()
                    .setDescription(`<:rei_idle:1218821750033350666> | Etiketlediğin kullanıcı **${sebep}** sebebiyle afk modunda!`)
